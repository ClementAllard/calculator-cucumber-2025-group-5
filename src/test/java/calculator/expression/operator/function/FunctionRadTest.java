...
class FunctionRadTest {
    ...
    void testRad(){
        try {
            Expression expression = new FunctionRad(myInteger);
            assertEquals("0.1570796327",calculator.eval(expression));

            expression = new FunctionRad(myReal);
            assertEquals("0.0087266463",calculator.eval(expression));

            expression = new FunctionRad(myRational);
            assertEquals("0.0087266463",calculator.eval(expression));

            expression = new FunctionRad(myComplex);
