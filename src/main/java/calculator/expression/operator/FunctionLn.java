...
public class FunctionLn extends Function{
    ...
    public FunctionLn(List<Expression> elist, String functionName) throws IllegalConstruction {
    ...
    protected MyNumber op(MyInteger l) {
        ...
        if (l.getValue() > 0){
            return new MyReal(BigDecimal.valueOf(Math.log(l.getValue())));
        } else {
    ...
    protected MyNumber op(MyRational l) {
        if (l.getReal().compareTo(BigDecimal.ZERO) > 0){
            return new MyReal(BigDecimal.valueOf(Math.log(l.getReal().doubleValue())));
        } else {
            throw new IllegalArgumentException(NEGATIVE_ERROR_STRING);
        }
    }
    ...
    protected MyNumber op(MyReal l) {
        if (l.getValue().compareTo(BigDecimal.ZERO) > 0){
            return new MyReal(BigDecimal.valueOf(Math.log(l.getValue().doubleValue())));
        } else {
            throw new IllegalArgumentException(NEGATIVE_ERROR_STRING);
        }
    }
